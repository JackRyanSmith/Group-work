...
const apiKey5 = "emVNjF05cHjztBVt";
...
async function searchForTeamData(city) {
  try {
    const responses = await Promise.all([
      fetch(
        `https://cors-anywhere.herokuapp.com/http://api.isportsapi.com/sport/basketball/team/search?api_key=${apiKey5}&name=${city}`
      ),
      fetch(
        `https://cors-anywhere.herokuapp.com/http://api.isportsapi.com/sport/basketball/schedule?api_key=${apiKey5}&leagueId=111`
      ),
    ]);

    const [teamSearchResults, schedule] = await Promise.all(
...
function displayTeamSelection() {
  ...
  for (var i = 0; i < teams.length; i++) {
    const divTemp = $("<div>");
    const teamFormItem = $("<input>")
      .attr("class", "choseTeam")
      .attr("type", "checkbox")
      .attr("id", "team" + [i])
      .attr("name", "team" + [i])
      .attr("value", `${teams[i].name}`)
      .appendTo(divTemp);
    const teamFormLabel = $("<label>")
      .attr("for", "team" + [i])
      .text(`${teams[i].name}`)
      .appendTo(divTemp);
...
function displayDataSelection() {
  ...
  const optionBody = $("<div>")
    .addClass("#option-card-body")
    .appendTo(optionItem);
  ...
  const divTemp1 = $("<div>");
  const optionItemOne = $("<input>")
    .attr("class", "choose-option")
    .attr("type", "checkbox")
    .attr("id", "option-1")
    .attr("name", "option-1")
    .appendTo(divTemp1);
  const optionOneLabel = $("<label>")
    .attr("for", "option-1")
    .text("Next Game")
    .appendTo(divTemp1);
  ...
  const divTemp2 = $("<div>");
  const optionItemTwo = $("<input>")
    .attr("class", "choose-option")
    .attr("type", "checkbox")
    .attr("id", "option-2")
    .attr("name", "option-2")
    .appendTo(divTemp2);
  const optionTwoLabel = $("<label>")
    .attr("for", "option-2")
    .text("Team Schedule")
    .appendTo(divTemp2);
  ...
  const divTemp3 = $("<div>");
  const optionItemThree = $("<input>")
    .attr("class", "choose-option")
    .attr("type", "checkbox")
    .attr("id", "option-3")
    .attr("name", "option-3")
    .appendTo(divTemp3);
  const optionThreeLabel = $("<label>")
    .attr("for", "option-3")
    .text("Team Standings")
    .appendTo(divTemp3);
  ...
  const submitBtn2 = $("<button>Submit</button>")
    .attr("id", "submit-btn")
    .addClass("btn waves-effect waves-light")
    .attr("type", "submit")
    .attr("name", "action")
    .appendTo(optionsForm);
  ...
  submitBtn2.on("click", function (event) {
    ...
    displayData(
      optionItemOne.is(":checked"),
      optionItemTwo.is(":checked"),
      optionItemThree.is(":checked")
    );
  });
}
...
function displayData(nextGame, teamSchedule, teamStanding) {
  ...
  teams.forEach(function (team) {
    ...
    const teamData = $("<div>")
      .text(team.name)
      .attr("id", "team-result")
      .appendTo(finalResults);
    if (nextGame) {
      const firstMatch = matches.find(function (match) {
        console.log(match.homeName);
        return (
          (match.homeName === team.name || match.awayName === team.name) &&
          new Date(match.matchTime * 1000) > new Date()
        );
      });
      const optionsData = $("<div>")
        .attr("id", "next-match-result")
        .text((firstMatch && new Date(firstMatch.matchTime * 1000)) || "none")
        .appendTo(teamData);
    }
    if (teamSchedule) {
      ...
      for (var i = 0; i < teamMatches.length; i++) {
        const optionsData = $("<div>")
          .attr("id", "match-results")
          .text(
            `${new Date(teamMatches[i].matchTime * 1000).toLocaleDateString(
              "en-US"
            )} ${teamMatches[i].awayName}: ${teamMatches[i].awayScore} ${
              teamMatches[i].homeName
            }: ${teamMatches[i].homeScore}`
          )
          .appendTo(schedResults);
      }
    }
    if (teamStanding) {
      // another api?
